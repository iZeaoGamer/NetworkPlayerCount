...
class FetchPlayerAsyncTask extends AsyncTask
{
	...
	public function onRun(){
		$playercount = 0;
		$config = (new Config(Main::getConfig()->get("waterdog-config-path", "/home/mcpe/Proxy/config.yml"), Config::YAML))->getAll();
		$servers = $config["servers"];

		foreach ($servers as $server) {
			$ex = explode(":", $server["address"]);
			$ip = str_replace("0.0.0.0", Main::getConfig()->get("server-address", "stimomc.de"), $ex[0]);
			$port = $ex[1];
